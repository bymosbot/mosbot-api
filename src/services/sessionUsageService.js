...
/**
 * Extract model string from a session object.
...
function extractModel(s) {
  if (s.modelProvider && s.model) {
    return `${s.modelProvider}/${s.model}`;
  }
...
async function upsertSessionUsageBatch(sessions) {
  ...
  try {
    ...
    for (const s of sessions) {
      ...
      const deltaCacheRead  = Math.max(0, newCacheRead  - (prev ? Number(prev.tokens_cache_read)  : 0));
      const deltaCacheWrite = Math.max(0, newCacheWrite - (prev ? Number(prev.tokens_cache_write) : 0));
