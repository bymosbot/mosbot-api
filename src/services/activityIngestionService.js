...
async function ingestCronAndHeartbeatRuns() {
  try {
    ...
    logger.error('[activityIngestion] Cron/heartbeat ingestion error', { error: err.message });
  }
}
...
async function ingestSubagentActivity() {
  try {
    ...
    for (const { entry, dedupeKey } of toIngest) {
      ...
      if (category.includes('completed') || category.includes('result') || category.includes('done')) {
      ...
      await recordActivityLogEvent({
        ...
        title: sessionLabel
          ...
          : (taskId ? `Subagent for task ${taskId}` : 'Subagent activity'),
    ...
    logger.error('[activityIngestion] Subagent ingestion error', { error: err.message });
  }
}
...
function startCronIngestionPoller(intervalMs = 2 * 60 * 1000) {
  ...
  logger.info('[activityIngestion] Starting cron/heartbeat ingestion poller', { intervalMs });
...
function startSubagentIngestionPoller(intervalMs = 3 * 60 * 1000) {
  ...
  logger.info('[activityIngestion] Starting subagent ingestion poller', { intervalMs });
...
function startActivityIngestionPollers(opts = {}) {
  const {
    cronIntervalMs = parseInt(process.env.ACTIVITY_CRON_POLL_INTERVAL_MS || '120000', 10),
    subagentIntervalMs = parseInt(process.env.ACTIVITY_SUBAGENT_POLL_INTERVAL_MS || '180000', 10),
  } = opts;
