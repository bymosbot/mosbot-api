...
describe('GET /api/v1/tasks/:id/subagents', () => {
  ...
  beforeAll(() => {
    ...
    mockGatewayUrl = 'http://mock-gateway:18789';
    process.env.OPENCLAW_WORKSPACE_URL = mockWorkspaceUrl;
    process.env.OPENCLAW_GATEWAY_URL = mockGatewayUrl;
  });
  ...
  describe('Happy path: runtime + gateway merge', () => {
    it('should return task-scoped subagents with runtime and gateway data merged', async () => {
      ...
      global.fetch = jest.fn().mockImplementation(async (url, options) => {
        ...
        // Mock gateway /tools/invoke
        if (url.includes('mock-gateway') && url.includes('/tools/invoke')) {
              ...
              result: [
                {
                  ...
                  kind: 'other',
                  model: 'sonnet',
                  abortedLastRun: true // Maps to failed
                }
              ]
            };
            return {
  ...
  describe('Error handling', () => {
    ...
    it('should return 401 when no token provided', async () => {
      const response = await request(app)
        .get('/api/v1/tasks/task-123/subagents');
