...
const validateUUID = (paramName) => (req, res, next) => {
  ...
  if (!uuidRegex.test(req.params[paramName])) {
    return res.status(400).json({ error: { message: `Invalid UUID for parameter '${paramName}'`, status: 400 } });
  }
  next();
};
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO standups (standup_date, title, timezone, status, started_at, completed_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [standup_date, title.trim(), timezone.trim(), resolvedStatus, started_at || null, completed_at || null]
    );
...
// ---------------------------------------------------------------------------
router.patch('/:id', authenticateToken, requireAdmin, validateUUID('id'), async (req, res, next) => {
  try {
    const { id } = req.params;
    const { standup_date, title, timezone, status, started_at, completed_at } = req.body;
    ...
    if (timezone !== undefined) {
      if (String(timezone).trim() === '') {
        return res.status(400).json({ error: { message: 'timezone cannot be empty', status: 400 } });
      }
    ...
    logger.error('Failed to update standup', { userId: req.user.id, standupId: req.params.id, error: error.message });
    next(error);
  }
});
...
// ---------------------------------------------------------------------------
router.delete('/:id', authenticateToken, requireAdmin, validateUUID('id'), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    logger.error('Failed to delete standup', { userId: req.user.id, standupId: req.params.id, error: error.message });
    next(error);
  }
});
...
// ---------------------------------------------------------------------------
router.post('/:id/run', authenticateToken, requireAdmin, validateUUID('id'), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    logger.info('Standup run completed', { userId: req.user.id, standupId: id, agentCount: result.agentCount });
    res.json({ data: updated.rows[0] });
  } catch (error) {
    logger.error('Failed to run standup', { userId: req.user.id, standupId: req.params.id, error: error.message });
    next(error);
  }
});
...
// ---------------------------------------------------------------------------
router.post('/:id/entries', authenticateToken, requireAdmin, validateUUID('id'), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    if (turn_order !== undefined && (isNaN(parseInt(turn_order, 10)) || parseInt(turn_order, 10) < 1)) {
      return res.status(400).json({ error: { message: 'turn_order must be a positive integer', status: 400 } });
    }
    ...
    const tasksValue = tasks !== undefined
    ...
    const result = await pool.query(
      `INSERT INTO standup_entries (
       ...
       RETURNING *`,
      [id, agent_id.trim(), user_id || null, resolvedTurnOrder, yesterday || null, today || null, blockers || null, tasksValue, raw.trim()]
    );

    logger.info('Standup entry created', { userId: req.user.id, standupId: id, entryId: result.rows[0].id });
    res.status(201).json({ data: result.rows[0] });
  } catch (error) {
    logger.error('Failed to create standup entry', { userId: req.user.id, standupId: req.params.id, error: error.message });
    next(error);
  }
});
    ...
    try {
      ...
      if (agent_id !== undefined) {
        if (String(agent_id).trim() === '') {
          return res.status(400).json({ error: { message: 'agent_id cannot be empty', status: 400 } });
        }
...
router.get('/:id/messages', authenticateToken, validateUUID('id'), async (req, res, next) => {
  try {
    ...
    logger.error('Failed to fetch standup messages', { userId: req.user.id, standupId: req.params.id, error: error.message });
    next(error);
  }
});
...
// ---------------------------------------------------------------------------
router.post('/:id/messages', authenticateToken, requireAdmin, validateUUID('id'), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    logger.info('Standup message created', { userId: req.user.id, standupId: id, messageId: result.rows[0].id });
    res.status(201).json({ data: result.rows[0] });
  } catch (error) {
    logger.error('Failed to create standup message', { userId: req.user.id, standupId: req.params.id, error: error.message });
    next(error);
  }
});
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      if (result.rows.length === 0) {
        return res.status(404).json({ error: { message: 'Standup message not found', status: 404 } });
      }
