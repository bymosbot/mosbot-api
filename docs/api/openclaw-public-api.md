...
Daily executive standups collect progress reports from agent users (COO, CTO, CPO, CMO) via OpenClaw Gateway `sessions_send`. Standups are generated by a scheduled job (cron) and stored in the database. These endpoints allow clients to list and retrieve standup data.
...
8. **Optional**: Display daily standups (`GET /standups/latest` or `GET /standups/:id`). Standups are generated by a cron job; use `GET /api/v1/config` for instance timezone.
